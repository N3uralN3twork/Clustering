...
/**
 * @brief Benchmark fixture for the Utils::get_centroids function.
 *
 * This function generates test data based on benchmark arguments and
 * times the execution of Utils::get_centroids.
 *
...
static void BM_get_centroids(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        Eigen::MatrixXd result = Utils::get_centroids(data, labels);
    ...
    state.counters["Num. Rows"] = num_samples;
    state.counters["Num. Features"] = num_features;
    state.counters["Num. Clusters"] = num_clusters;
}


/**
 * @brief Benchmark fixture for the Utils::gss function.
 *
 * This function generates test data based on benchmark arguments and
 * times the execution of Utils::gss.
 *
 * The `state` object is managed by Google Benchmark.
 * `state.range(0)` = Number of Samples
 * `state.range(1)` = Number of Features
...
static void BM_GSS(benchmark::State& state) {
    ...
    for (auto _ : state) {
        // Call the function we want to test
        Utils::GssResult result = Utils::gss(data, labels);
    ...
    state.counters["Num. Rows"] = num_samples;
    state.counters["Num. Feats"] = num_features;
    state.counters["Num. Clusters"] = num_clusters;
}
